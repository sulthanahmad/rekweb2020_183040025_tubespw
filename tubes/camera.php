...
$camera = query("SELECT * FROM barang WHERE kategori LIKE '%Camera%' ");
 ?>
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb" style="font-size: 17px;">
    ...
    <li class="breadcrumb-item active" aria-current="page">Camerar</li>
  </ol>
</nav>


<!-- tables -->
<h5 style="margin-top: 40px; margin-left: 60px;"> Camera </h5>
<hr style="width: 100px; background-color: black; height: 2px; margin-left: 55px; margin-top: 10px;">
<?php foreach ($camera as $c) : ?>
 <div class="card imgbaru" style="width: 18rem; margin-left: 77px; margin-top: 20px; float: left;"> 
   <a href="tampil.php?foto=<?= $c['foto'];?>&nama=<?=  $c['nama']; ?>&deskripsi=<?=  $c['deskripsi']; ?>&speksifikasi=<?=  $c['speksifikasi']; ?>&brand=<?=  $c['brand'];?>&harga=<?= $c['harga'];?>"><img src="assets/imgbaru/<?= $c['foto']; ?>" class="card-img-top" height="250px" width="200px"></a>
  <div class="card-body" style="height: 60px;">
    <p class="card-text" style="margin-top: -12px;"><?= $c['nama']; ?></p>
  </div>
</div>
<?php endforeach; ?>
