...
$earphone = query("SELECT * FROM barang WHERE kategori LIKE '%headphone%' ");
 ?>
...
<nav class="navbar navbar-expand-lg navbar-dark nav" style="font-size: 17px;">
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    ...
    <form class="form-inline my-2 my-lg-0">
      <input class="form-control mr-sm-2" type="search" placeholder="Search" id="cari" name="cari" aria-label="Search">
      <button class="btn btn-light btn-outline-success border-white my-2 my-sm-0 mr-sm-2" type="submit">Search</button></a>
...
<!-- tables -->
<h5 style="margin-top: 40px; margin-left: 60px;"> Headphone </h5>
...
<?php foreach ($earphone as $e) : ?>
 <div class="card imgbaru" style="width: 18rem; margin-left: 77px; margin-top: 20px; float: left;"> 
   <a href="tampil.php?foto=<?= $e['foto'];?>&nama=<?=  $e['nama']; ?>&deskripsi=<?=  $e['deskripsi']; ?>&speksifikasi=<?=  $e['speksifikasi']; ?>&brand=<?=  $e['brand'];?>&harga=<?= $e['harga'];?>"><img src="assets/imgbaru/<?= $e['foto']; ?>" class="card-img-top" height="250px" width="200px"></a>
  <div class="card-body" style="height: 60px;">
    <p class="card-text" style="margin-top: -12px;"><?= $e['nama']; ?></p>
  </div>
</div>
<?php endforeach; ?>
