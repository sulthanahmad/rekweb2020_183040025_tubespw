...
$Kulkas = query("SELECT * FROM barang WHERE kategori LIKE '%Kulkas%' ");
 ?>
...
<!-- tables -->
<h5 style="margin-top: 40px; margin-left: 60px;"> Kulkas </h5>
<hr style="width: 100px; background-color: black; height: 2px; margin-left: 55px; margin-top: 10px;">
<?php foreach ($Kulkas as $a) : ?>
 <div class="card imgbaru" style="width: 18rem; margin-left: 77px; margin-top: 20px; float: left;"> 
   <a href="tampil.php?foto=<?= $a['foto'];?>&nama=<?=  $a['nama']; ?>&deskripsi=<?=  $a['deskripsi']; ?>&speksifikasi=<?=  $a['speksifikasi']; ?>&brand=<?=  $a['brand'];?>&harga=<?= $a['harga'];?>"><img src="assets/imgbaru/<?= $a['foto']; ?>" class="card-img-top" height="250px" width="200px"></a>
  <div class="card-body" style="height: 60px;">
    <p class="card-text" style="margin-top: -12px;"><?= $a['nama']; ?></p>
  </div>
</div>
<?php endforeach; ?>
