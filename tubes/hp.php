...
$barang = query("SELECT * FROM barang WHERE kategori LIKE '%handphone%'");
...
<!-- tables -->
<h5 style="margin-top: 40px; margin-left: 60px;"> Handphone </h5>
...
<?php foreach ($barang as $h) : ?>
 <div class="card imgbaru" style="width: 18rem; margin-left: 77px; margin-top: 20px; float: left;"> 
   <a href="tampil.php?foto=<?= $h['foto'];?>&nama=<?=  $h['nama']; ?>&deskripsi=<?=  $h['deskripsi']; ?>&speksifikasi=<?=  $h['speksifikasi']; ?>&brand=<?=  $h['brand'];?>&harga=<?= $h['harga'];?>"><img src="assets/imgbaru/<?= $h['foto']; ?>" class="card-img-top" height="250px" width="200px"></a>
  <div class="card-body" style="height: 60px;">
    <p class="card-text" style="margin-top: -12px;"><?= $h['nama']; ?></p>
  </div>
</div>
<?php endforeach; ?>
