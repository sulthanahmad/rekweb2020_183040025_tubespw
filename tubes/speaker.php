...
$speaker = query("SELECT * FROM barang WHERE kategori LIKE '%Speaker%' ");
 ?>
...
<!-- tables -->
<h5 style="margin-top: 40px; margin-left: 60px;"> Speaker </h5>
...
<?php foreach ($speaker as $s) : ?>
 <div class="card imgbaru" style="width: 18rem; margin-left: 77px; margin-top: 20px; float: left;"> 
   <a href="tampil.php?foto=<?= $s['foto'];?>&nama=<?=  $s['nama']; ?>&deskripsi=<?=  $s['deskripsi']; ?>&speksifikasi=<?=  $s['speksifikasi']; ?>&brand=<?=  $s['brand'];?>&harga=<?= $s['harga'];?>"><img src="assets/imgbaru/<?= $s['foto']; ?>" class="card-img-top" height="250px" width="200px"></a>
  <div class="card-body" style="height: 60px;">
    <p class="card-text" style="margin-top: -12px;"><?= $s['nama']; ?></p>
  </div>
</div>
<?php endforeach; ?>
